...
export default class SkillList extends PureComponent {
	...
	handleEditItem = () => {
		...
		this.handleSkillUpdate({
			...
			level: parseInt(this.proficiancy.value, 10),
		});
	};

	handleSkillUpdate = update => {
		this.props.update("skills", update);
		this.setState(() => ({
			editing: null,
		}));
	};
	...
	renderItemOrEditField = item => {
		...
		return (
			<tr key={item._id} onClick={() => this.toggleEditing(item._id)}>
				...
				<td>{item.level}</td>
				<td>
					<button className="btn btn-default" onClick={() => this.remove(item)}>remove</button>
				</td>
			</tr>
		);
	};
